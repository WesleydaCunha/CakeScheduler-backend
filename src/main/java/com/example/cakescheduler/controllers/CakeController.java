...
import com.example.cakescheduler.domain.cake.*;
import com.example.cakescheduler.dto.*;
...
public class CakeController {
    ...
    @Autowired
    private CakeModelCategoryRepository cakeModelCategoryRepository;
    @Autowired
    ...
    public ResponseEntity<List<CakeModel>> getCakeModelsByCategory(@RequestParam(required = false) String category) {
        List<CakeModel> listModels;
        if (category != null) {
            listModels = cakeModelRepository.findByCategoryCategoryName(category);
        } else {
            listModels = cakeModelRepository.findAll();
        }
        return ResponseEntity.ok(listModels);
    }
    ...
    public ResponseEntity<String> saveCakeModel(@RequestBody CakeModelRequestDTO body) {
        ...
        Optional<CakeModelCategory> cakeModelCategory = cakeModelCategoryRepository.findById(body.category());
        if (cakeModelCategory.isEmpty()) {
            return ResponseEntity.badRequest().body("Categoria de bolo não encontrada.");
        }
        ...
        try {
            ...
            newCakeModel.setCategory(cakeModelCategory.get());
            this.cakeModelRepository.save(newCakeModel);
            return ResponseEntity.ok("Modelo de bolo registrado com sucesso!");
        } catch (Exception e) {
            e.printStackTrace(); // Considerar usar um logger aqui
            return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Erro ao registrar o modelo de bolo. Tente novamente mais tarde.");
        }
    }


    @PostMapping("/category/register")
    public ResponseEntity<String> saveCakeModelCategory(@RequestBody CakeModelCategoryRequestDTO body) {
        try {
            CakeModelCategory newCakeModelCategory = new CakeModelCategory();
            newCakeModelCategory.setCategory_name(body.category_name());
            this.cakeModelCategoryRepository.save(newCakeModelCategory);
            return ResponseEntity.ok("Sucesso!");
        } catch (Exception e) {
            e.printStackTrace();
            return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Erro ao registrar a categoria");
        }
    }

    @GetMapping("/category")
    public ResponseEntity<List<CakeModelCategory>> getAllCakeModelsCategory() {
        List<CakeModelCategory> listModelsCategory = cakeModelCategoryRepository.findAll();
        return ResponseEntity.ok(listModelsCategory);
    }

    @DeleteMapping("/category/{id}")
    public ResponseEntity<String> deleteCakeModelCategory(@PathVariable Long id) {
        Optional<CakeModelCategory> existingCakeModelCategory = cakeModelCategoryRepository.findById(id);

        if (existingCakeModelCategory.isPresent()) {
            cakeModelCategoryRepository.delete(existingCakeModelCategory.get());
    ...
    public ResponseEntity<String> updateCakeModel(@PathVariable Long id, @RequestBody CakeModelRequestDTO body) {
        ...
        if (existingCakeModel.isPresent()) {
            ...
            cakeModel.setCategory(cakeModelCategory.get()); // Correção aqui
    ...
    public ResponseEntity<String> registerCakeComplement(@RequestBody ComplementRequestDTO body) {
        ...
        newComplement.setUrl(body.image_url());
    ...
    public ResponseEntity<String> updateComplement(@PathVariable Long id, @RequestBody ComplementRequestDTO body) {
        ...
        if (existingComplement.isPresent()) {
            ...
            complement.setUrl(body.image_url());
